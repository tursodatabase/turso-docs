...
Make sure to call [`.sync()`](#sync) to synchronize the embedded replica with the remote database.
...
The `sync()` method can be called when [Embedded Replicas](/embedded-replicas) are configured to sync changes.
