...
```bash Request
curl -L -X GET 'https://[databaseName]-[organizationName].turso.io/dump' \
  -H 'Authorization: Bearer TOKEN'
```
...
Turso provides a managed multi-tenant schema system &mdash; [multi-db schemas](/features/multi-db-schemas). You can create a shared schema for other databases and when the parent database schema is updated, changes are propogated to the child databases.

The `/jobs` endpoint lets you check the status of those migrations.

<Warning>Not available on AWS.</Warning>
...
```bash Request
curl -X GET 'https://[databaseName]-[organizationName].turso.io/v1/jobs' \
     -H 'Authorization: Bearer TOKEN'
```
...
```bash Request
curl -X GET 'https://[databaseName]-[organizationName].turso.io/v1/jobs/:id' \
     -H 'Authorization: Bearer TOKEN'
```
...
```json Example Response
{
  ...
  "progress": [
    {
      "namespace": "28211856-d584-402d-869b-a947306f648d",
      "status": "RunSuccess",
      "error": null
    },
    {
      "namespace": "911b6376-448f-4426-9aa5-9799fec228b1",
      "status": "RunSuccess",
      "error": null
    },
    {
      "namespace": "d6cf31c8-735b-46b5-af6e-3bc5207eb75e",
      "status": "RunSuccess",
      "error": null
    }
  ]
...
```bash Request
curl -L 'https://[primary-instance-id]-[databaseName]-[organizationName].turso.io/beta/listen?table=TABLE_NAME' \
-H 'Authorization: Bearer TOKEN'
```
