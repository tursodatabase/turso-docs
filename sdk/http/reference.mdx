...
```bash Request
curl -L 'https://[primary-instance-id]-[databaseName]-[organizationName].turso.io/beta/listen?table=TABLE_NAME&action=insert' \
-H 'Authorization: Bearer TOKEN'
```

#### Query Parameters

- `table` (required) &mdash; The name of the table to listen to.
- `action` (optional) &mdash; The action to listen to, `insert`, `update`, or `delete`.

<Info>
  Your database group must be using the version `v0.24.18` or greater &mdash; `turso group update <group-name> --version v0.24.18`
</Info>
