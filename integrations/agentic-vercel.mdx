...
**Turso Cloud** is a fully managed SQLite-compatible database platform. Databases provisioned through the Vercel Marketplace are Turso Cloud databases accessed over the network.
...
For apps that need fast local reads, offline support, or embedded replicas that sync with Turso Cloud. Sync is explicit (`push()`/`pull()`), conflict resolution is last-push-wins, and a synced database file must **never** be opened outside the sync SDK. Besides the JavaScript family (`@tursodatabase/sync`, `@tursodatabase/sync-wasm`, `@tursodatabase/sync-react-native`), sync SDKs are available for Python (`pyturso`) and Go (`tursogo`) — all of them work seamlessly with databases provisioned in the cloud. See `https://docs.turso.tech/sync/usage.md` for setup, API, and examples.
...
| Use case | Package | Install |
|----------|---------|---------|
| Local-first + sync (Python) | `pyturso` | `pip install pyturso` |
| Local-first + sync (Go) | `tursogo` | `go get turso.tech/database/tursogo` |
| Remote-only (Python, libsql protocol) | `libsql` | `pip install libsql` |
| Remote-only (Go, libsql protocol) | `github.com/tursodatabase/libsql-client-go` | `go get github.com/tursodatabase/libsql-client-go` |
| Remote-only (Rust, libsql protocol) | `libsql` | `cargo add libsql` |
