...
After `env --sync`, the `.env` file will contain `TURSO_DATABASE_URL` and `TURSO_AUTH_TOKEN` ready for use with `@libsql/client` or any Turso SDK.
...
<Warning>
**Database removal is irreversible and causes permanent data loss.** The database and all its data are destroyed immediately. There is no undo, no recycle bin, no recovery period. An agent must NEVER remove a database without explicit, unambiguous confirmation from the user for that specific database by name.
</Warning>

When the user explicitly confirms removal of a specific database:

```bash
stripe projects remove <resource-name> --yes
```

### Removing a plan

Removing a plan resource downgrades the account to the free `starter` tier. This may cause services to exceed the starter quotas, potentially resulting in throttling or data becoming inaccessible. Always warn the user about these consequences before proceeding.
...
| Operation | Autonomous | Requires user consent |
| --- | --- | --- |
| `stripe projects init` | Yes | |
| `stripe projects status` | Yes | |
| `stripe projects catalog turso` | Yes | |
| `stripe projects add turso/database` | Yes | |
| `stripe projects env` / `env --sync` / `env --refresh` | Yes | |
| `stripe projects add turso/<paid-plan>` | | Yes — billing impact |
| `stripe projects upgrade` | | Yes — billing impact |
| `stripe projects billing update` | | Yes — payment method change |
| `stripe projects remove` (database) | | Yes — **irreversible data loss** |
| `stripe projects remove` (plan) | | Yes — downgrade consequences |
| `stripe projects rotate` | | Yes — **invalidates all existing tokens** |
