...
Pass the acquired credentials to the flutter app by using the `--dart-define` flags:

```bash
flutter run --debug \
  --dart-define=TURSO_URL=[issued-db-url] \
  --dart-define=TURSO_AUTH_TOKEN=[issued-db-token]
```
...
Future<List<T>> getItems() async {
  final response = await http.post(Uri.parse(tursoDbUrl),
      ...
      body: json.encode({
        "statements": ["select * from my-table"]
      }));
...
[Have a Flutter project]: https://docs.flutter.dev/get-started/codelab
