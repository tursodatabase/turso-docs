...
Turso Cloud provides native encryption for data at rest using industry-standard AEAD algorithms. Each database page is encrypted individually, ensuring data integrity and security across all environments. Database encryption is available on the [Enterprise plan](https://tur.so/turso-enterprise-chat).
...
<CardGroup cols={2}>
    <Card title="AI Agents" icon="robot">
      Protect agent memory, tool results, and sensitive context between runs
    </Card>
