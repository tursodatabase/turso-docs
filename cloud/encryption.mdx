...
For Pro, and Enterprise customers, there is the option to encrypt the databases with a key that you provide (Native Encryption), as described below.
...
Turso Cloud native encryption for data at rest uses industry-standard AEAD algorithms. Each database page is encrypted individually, ensuring data integrity and security across all environments. Native encryption is available on Pro, and [Enterprise](https://tur.so/turso-enterprise-chat) plans.
