...
<Note>
  Pro customers never share data with databases from other organizations. For
  the Free, Developer, and Scaler tier, commit data is shared across databases
  from other organizations, but encrypted individually with a per-organization
  key.
</Note>

## Offline writes durability guarantees

Turso databases can work in offline modes, where writes happen in a local SQLite file, and later sent to Turso Cloud. This is achieved by calling the `sync()` method on the database object. Turso Cloud standard durability guarantees and commit latency apply after the `sync()` method is called.
